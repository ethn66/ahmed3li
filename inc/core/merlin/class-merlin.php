...
class Merlin {
	...
	protected function content() {
		...
		<?php echo wp_kses( $this->svg( array( 'icon' => 'content' ) ), $this->svg_allowed_html() ); ?>
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				$req_mem = 256;
				$req_exe = 300;
				if ( $max_execute < $req_exe || $memory_limit < $req_mem ) {
					echo '<div id="server-health-info">';
					echo '<h3>Server Health</h3>';
					echo '<span id="health-meter" class="does-not-meet-requirements"></span>';
					echo '<p class="import__req__msg">You may experience timeout issues during this process. <a href="https://docs.codetipi.com/zeen/#minimum-server-requirements" target="_blank">Click here for more info</a>.</p>';
					echo '<ul class="server-info">';
					echo '<li><span class="server-feature">PHP Memory Limit</span> <span class="server-value">';
					if ( $memory_limit < $req_mem ) {
						echo '<span class="below-req">' . esc_html( $memory_limit ) . '</span>';
					} else {
						echo '<span class="meets-req">' . esc_html( $memory_limit ) . '</span>';
					}
					echo '</span>';
					if ( $memory_limit < $req_mem ) {
						echo '<span class="recommended-req">' . esc_html__( 'Recommended', 'zeen' ) . ' <span class="server-value">' . esc_html( $req_mem ) . 'M</span></span>';
					}
					echo '</li>';
					echo '<li><span class="server-feature">PHP Max Execution Time</span> <span class="server-value">';
					if ( $max_execute < $req_exe ) {
						echo '<span class="below-req">' . esc_html( $max_execute ) . 's</span>';
					} else {
						echo '<span class="meets-req">' . esc_html( $max_execute ) . 's</span>';
					}
					echo '</span>';
					if ( $max_execute < $req_exe ) {
						echo '<span class="recommended-req">' . esc_html__( 'Recommended', 'zeen' ) . ' <span class="server-value">' . esc_html( $req_exe ) . 's</span></span>';
					}
						echo '</li>';
		...
		<form action="" method="post">
			...
			<ul class="merlin__drawer merlin__drawer--import-content js-merlin-drawer-import-content">
					<?php $this->get_import_steps_html( $import_info, true ); ?>
			</ul>
	...
	public function update_selected_import_data_info() {
		 ...
		 * @return string
		 */
	public function get_import_steps_html( $import_info, $echo = '' ) {
		ob_start();
		?>
